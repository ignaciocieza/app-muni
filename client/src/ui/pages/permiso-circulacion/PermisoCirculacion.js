...
import {
  setCurrentImage,
  setUserStart,
} from "../../../api/actions/user/userActions";
import {
  setAlerts,
  setIsFetching,
  setIsHeader,
...
import {
  ...
  isTelefono,
  isMail,
  isValidSubmitCirculacion,
} from "../permisos/validation";
import {
  TextField,
  Button,
  Icon,
...
const PermisoCirculacion = () => {
  ...
  const { currentImage, admin, isFetching, alerts, error } = useSelector(
    (state) => state.user
  );
  ...
  const handleSubmit = (event) => {
    ...
    setErrorValues({
      ...errorValues,
      permisoTipo: !userValues.permisoTipo,
      permiso: !userValues.permiso || userValues.permiso === "PENDIENTE",
    });
    ...
    if (!isValidArray.length) {
      ...
      dispatch(
        setUserStart({ ...userValues, image: currentImage, isRedirect: true })
      );
      // if (admin) {
  ...
  return (
    <form className={classes.form} onSubmit={handleSubmit} autoComplete="off">
      {isFetching && <Modal />}
      {error && !isFetching && (
        <Modal timeOut={6000} comentTitle="Error!" comentSubtitle={error} />
      )}
      ...
      <ImagePicker
        title="* ADJUNTAR IMAGEN DEL FRENTE DEL DNI"
        dispatch={(e) => {
